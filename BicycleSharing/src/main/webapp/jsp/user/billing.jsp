...
<body>
	...
	<div class="info">
		<div style="color: #a94442" class="text-center">
			<c:if test="${card_error =='true'}">
				<p>${card_create_error}</p>
			</c:if>
		</div>
		<div class="container">
			<div class="row">
				<div class="col-sm-4">
					...
					<form name="form" method="POST" action="${pageContext.session.servletContext.contextPath}/controller" id="card">
						...
						<button type="submit" class="btn btn-success btn-block" id="adding">${add_card}</button>
					</form>
				</div>
				<div class="col-sm-8">
					<h3>${card_table}</h3>
								...
								<tr>
										...
										<c:choose>
												<c:when test='${num.getStatus() == "DISABLE"}'>
													<td><a href="${pageContext.session.servletContext.contextPath}/controller?command=Enable_card&id=${num.getId()}"><i class="glyphicon glyphicon-check"></i></a></td>
												</c:when>
												<c:when test='${num.getStatus() == "ENABLE"}'>
													<td><a href="${pageContext.session.servletContext.contextPath}/controller?command=Disable_card&id=${num.getId()}"><i class="glyphicon glyphicon-Lock"></i></a></td>
												</c:when>
											</c:choose>
									</tr>
								</c:forEach>
				...
				<div id="bill-wrapper">
						<div class="bill-inner">
					<div class="tab">
						<form name="form" method="POST" action="${pageContext.session.servletContext.contextPath}/controller"><table class="table table-hover">
								<tr>
									<th>${master}</th>
									<th>${balance}</th>
									<th></th>
